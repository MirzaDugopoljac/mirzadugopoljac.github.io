...
const additions = {
  ...
  'new.all': ['Explore all 6 projects ↗', '6 projenin tamamını incele ↗', 'Pogledaj svih 6 projekata ↗'],
...
for (const file of outputPages) {
  ...
  html=html.replace('</head>', `<link rel="stylesheet" href="${cssRoot}assets/css/exhibition.css"></head>`);
  html=html.replace('</body>', `<script src="${cssRoot}assets/js/effects.js"></script><script src="${cssRoot}assets/js/exhibition.js"></script></body>`);
  const dimensions=JSON.parse(fs.readFileSync('assets/img/dimensions.json','utf8'));
  html=html.replace(/<img\b[^>]*>/g,tag=>{
    const match=tag.match(/src="([^"]*assets\/img\/([^"/]+))"/);if(!match||!dimensions[match[2]])return tag;
    const [width,height]=dimensions[match[2]];
    tag=tag.replace(/ (?:width|height)="[^"]*"/g,'').replace('>',` width="${width}" height="${height}" decoding="async">`);
    if(/^gemlik-\d+\.webp$/.test(match[2]))tag=tag.replace('>',` srcset="${match[1].replace('.webp','-800.webp')} 800w, ${match[1]} ${width}w" sizes="(max-width: 760px) 90vw, 85vw">`);
    return tag;
  });
  if(file.endsWith('/gemlik.html'))html=html.replace('https://mirzadugopoljac.github.io/portfolio/assets/og-cover.jpg','https://mirzadugopoljac.github.io/assets/img/gemlik-09.webp');
  fs.writeFileSync(file, html.replace(/></g, '>\n<'));
}

require('./os-build.cjs')(outputPages);

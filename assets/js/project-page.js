(() => {
  ...
  function render(){const l=window.siteLang();
    ...
    document.getElementById('gallery').innerHTML=p.gallery.map(([img,cap])=>`<a href="${root}assets/img/${img}" data-lightbox><img src="${root}assets/img/${img}" alt="${cap[l]}" loading="lazy"><span class="caption">${cap[l]}</span></a>`).join('');

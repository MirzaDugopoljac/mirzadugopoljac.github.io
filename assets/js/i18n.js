window.I18N = {
  "nav.work": {
    "en": "Work",
    "tr": "Projeler",
    "bs": "Radovi"
  },
  "nav.about": {
    "en": "About",
    "tr": "Hakkımda",
    "bs": "O meni"
  },
  "nav.contact": {
    "en": "Contact",
    "tr": "İletişim",
    "bs": "Kontakt"
  },
  "home.kicker": {
    "en": "LANDSCAPE ARCHITECTURE · DESIGN · VISUALIZATION",
    "tr": "PEYZAJ MİMARLIĞI · TASARIM · GÖRSELLEŞTİRME",
    "bs": "PEJZAŽNA ARHITEKTURA · DIZAJN · VIZUALIZACIJA"
  },
  "home.title": {
    "en": "Read the land.<br>Shape what<br><em>comes next.</em>",
    "tr": "Araziyi oku.<br>Geleceği<br><em>şekillendir.</em>",
    "bs": "Čitaj prostor.<br>Oblikuj ono<br><em>što dolazi.</em>"
  },
  "home.copy": {
    "en": "Landscape architecture, from the first site reading to the spaces we share. I’m Mirza Dugopoljac, a student at Bursa Uludağ University working across design, GIS and visualization.",
    "tr": "İlk alan okumasından paylaştığımız mekânlara uzanan peyzaj mimarlığı. Ben Mirza Dugopoljac; Bursa Uludağ Üniversitesi’nde tasarım, CBS ve görselleştirme üzerine çalışan bir öğrenciyim.",
    "bs": "Pejzažna arhitektura, od prvog čitanja lokacije do prostora koje dijelimo. Ja sam Mirza Dugopoljac, student Univerziteta Bursa Uludağ, usmjeren na dizajn, GIS i vizualizaciju."
  },
  "home.ctaWork": {
    "en": "View selected work ↗",
    "tr": "Seçili projeleri incele ↗",
    "bs": "Pogledaj odabrane radove ↗"
  },
  "home.ctaAbout": {
    "en": "About my approach",
    "tr": "Yaklaşımım",
    "bs": "O mom pristupu"
  },
  "home.bandTitle": {
    "en": "A portfolio moving between site, system and atmosphere.",
    "tr": "Alan, sistem ve atmosfer arasında ilerleyen bir portfolyo.",
    "bs": "Portfolio koji se kreće između lokacije, sistema i atmosfere."
  },
  "home.bandText": {
    "en": "Different scales, one consistent question: what should change here, and why?",
    "tr": "Farklı ölçekler, aynı soru: burada ne değişmeli ve neden?",
    "bs": "Različite skale, isto pitanje: šta ovdje treba promijeniti i zašto?"
  },
  "home.section1": {
    "en": "01 / Approach",
    "tr": "01 / Yaklaşım",
    "bs": "01 / Pristup"
  },
  "home.section1Title": {
    "en": "Read first. <em>Design second.</em>",
    "tr": "Önce oku. <em>Sonra tasarla.</em>",
    "bs": "Prvo čitaj. <em>Onda dizajniraj.</em>"
  },
  "home.p1": {
    "en": "Understand the site.",
    "tr": "Alanı anla.",
    "bs": "Razumij lokaciju."
  },
  "home.p1p": {
    "en": "Terrain, movement, ecology and everyday use create the first design brief.",
    "tr": "Arazi, hareket, ekoloji ve gündelik kullanım ilk tasarım briefini oluşturur.",
    "bs": "Teren, kretanje, ekologija i svakodnevna upotreba čine prvi projektni zadatak."
  },
  "home.p2": {
    "en": "Test the idea.",
    "tr": "Fikri test et.",
    "bs": "Testiraj ideju."
  },
  "home.p2p": {
    "en": "Plans, sections, models and GIS help reveal whether a concept actually works.",
    "tr": "Plan, kesit, model ve CBS bir fikrin gerçekten çalışıp çalışmadığını test etmeye yardımcı olur.",
    "bs": "Planovi, presjeci, modeli i GIS pomažu provjeriti da li koncept zaista funkcioniše."
  },
  "home.p3": {
    "en": "Shape the experience.",
    "tr": "Deneyimi şekillendir.",
    "bs": "Oblikuj iskustvo."
  },
  "home.p3p": {
    "en": "Planting, materials, edges and circulation turn strategy into a lived spatial experience.",
    "tr": "Bitkiler, malzemeler, sınırlar ve dolaşım stratejiyi yaşanan bir mekânsal deneyime dönüştürür.",
    "bs": "Sadnja, materijali, rubovi i kretanje pretvaraju strategiju u doživljeno prostorno iskustvo."
  },
  "home.section2": {
    "en": "02 / Selected work",
    "tr": "02 / Seçili projeler",
    "bs": "02 / Odabrani radovi"
  },
  "home.section2Title": {
    "en": "Projects with <em>different rhythms.</em>",
    "tr": "<em>Farklı ritimlere</em> sahip projeler.",
    "bs": "Projekti sa <em>različitim ritmovima.</em>"
  },
  "home.footerCta": {
    "en": "See the work.<br><em>Then look closer.</em>",
    "tr": "Projeleri gör.<br><em>Sonra daha yakından bak.</em>",
    "bs": "Pogledaj radove.<br><em>Onda pogledaj bliže.</em>"
  },
  "home.footerText": {
    "en": "Explore the portfolio, read more about my approach, or get in touch.",
    "tr": "Portfolyoyu incele, yaklaşımım hakkında daha fazlasını oku veya iletişime geç.",
    "bs": "Pogledaj portfolio, pročitaj više o mom pristupu ili mi se javi."
  },
  "about.kicker": {
    "en": "ABOUT · LANDSCAPE ARCHITECTURE",
    "tr": "HAKKIMDA · PEYZAJ MİMARLIĞI",
    "bs": "O MENI · PEJZAŽNA ARHITEKTURA"
  },
  "about.title": {
    "en": "Curious about<br><em>how places work.</em>",
    "tr": "Mekânların<br><em>nasıl çalıştığını</em> merak ediyorum.",
    "bs": "Zanima me<br><em>kako mjesta funkcionišu.</em>"
  },
  "about.copy": {
    "en": "I’m a Landscape Architecture student at Bursa Uludağ University. My work moves between design, planning, planting, GIS and visualization, with a growing interest in how analysis becomes a strong spatial idea.",
    "tr": "Bursa Uludağ Üniversitesi’nde Peyzaj Mimarlığı öğrencisiyim. Çalışmalarım tasarım, planlama, bitkisel tasarım, CBS ve görselleştirme arasında ilerliyor; özellikle analizin güçlü bir mekânsal fikre nasıl dönüştüğüyle ilgileniyorum.",
    "bs": "Student sam pejzažne arhitekture na Univerzitetu Bursa Uludağ. Moj rad se kreće između dizajna, planiranja, sadnje, GIS-a i vizualizacije, uz sve veće interesovanje za to kako analiza postaje snažna prostorna ideja."
  },
  "about.section1": {
    "en": "01 / Position",
    "tr": "01 / Yaklaşım",
    "bs": "01 / Stav"
  },
  "about.section1Title": {
    "en": "The site should stay specific — even after <em>the intervention.</em>",
    "tr": "Müdahaleden sonra bile alan <em>kendine özgü</em> kalmalı.",
    "bs": "Lokacija treba ostati specifična — čak i nakon <em>intervencije.</em>"
  },
  "about.quote": {
    "en": "A good intervention should belong to the place, but still make the place <em>work differently.</em>",
    "tr": "İyi bir müdahale alana ait hissettirmeli; ama aynı zamanda alanın <em>farklı çalışmasını</em> sağlamalı.",
    "bs": "Dobra intervencija treba pripadati mjestu, ali istovremeno učiniti da mjesto <em>funkcioniše drugačije.</em>"
  },
  "about.quoteP": {
    "en": "I’m most interested in the moment when analysis becomes a spatial decision: when a path, planting system, edge or structure starts solving more than one problem at once.",
    "tr": "En çok analizin mekânsal karara dönüştüğü anla ilgileniyorum: bir yolun, bitki sisteminin, sınırın veya yapının aynı anda birden fazla probleme cevap verdiği an.",
    "bs": "Najviše me zanima trenutak kada analiza postane prostorna odluka: kada staza, sadnja, rub ili struktura počnu rješavati više problema odjednom."
  },
  "about.card1": {
    "en": "What I look for",
    "tr": "Ne arıyorum",
    "bs": "Šta tražim"
  },
  "about.card1b": {
    "en": "Patterns, constraints and opportunities hidden inside the site.",
    "tr": "Alan içinde saklı örüntüler, kısıtlar ve fırsatlar.",
    "bs": "Obrasce, ograničenja i prilike skrivene unutar lokacije."
  },
  "about.card2": {
    "en": "What I am building toward",
    "tr": "Neye doğru ilerliyorum",
    "bs": "Prema čemu idem"
  },
  "about.card2b": {
    "en": "A practice comfortable with terrain, drawings, analysis and visual communication.",
    "tr": "Arazi, çizim, analiz ve görsel iletişimi birlikte kullanan bir pratik.",
    "bs": "Praksu koja se jednako dobro snalazi s terenom, crtežima, analizom i vizualnom komunikacijom."
  },
  "about.section2": {
    "en": "02 / Working method",
    "tr": "02 / Çalışma yöntemi",
    "bs": "02 / Način rada"
  },
  "about.section2Title": {
    "en": "Read. Test. Refine. <em>Repeat.</em>",
    "tr": "Oku. Test et. Sadeleştir. <em>Tekrarla.</em>",
    "bs": "Čitaj. Testiraj. Doradi. <em>Ponovi.</em>"
  },
  "about.footerCta": {
    "en": "Learning through<br><em>real questions.</em>",
    "tr": "Gerçek sorular üzerinden<br><em>öğrenmek.</em>",
    "bs": "Učiti kroz<br><em>stvarna pitanja.</em>"
  },
  "about.footerText": {
    "en": "The projects keep changing, but the aim stays consistent: make the thinking clearer and the spatial result stronger.",
    "tr": "Projeler değişiyor; ama amaç aynı kalıyor: düşünceyi daha net, mekânsal sonucu daha güçlü hale getirmek.",
    "bs": "Projekti se mijenjaju, ali cilj ostaje isti: učiniti razmišljanje jasnijim, a prostorni rezultat snažnijim."
  },
  "contact.kicker": {
    "en": "CONTACT · COLLABORATION · CONVERSATION",
    "tr": "İLETİŞİM · İŞBİRLİĞİ · KONUŞMA",
    "bs": "KONTAKT · SARADNJA · RAZGOVOR"
  },
  "contact.title": {
    "en": "Have something<br><em>worth exploring?</em>",
    "tr": "Keşfetmeye değer<br><em>bir şey mi var?</em>",
    "bs": "Imaš nešto<br><em>vrijedno istraživanja?</em>"
  },
  "contact.copy": {
    "en": "A project, a site, a visualization question or simply an idea — send the context and we can start from there.",
    "tr": "Bir proje, alan, görselleştirme sorusu veya yalnızca bir fikir — bağlamı gönder, oradan başlayalım.",
    "bs": "Projekat, lokacija, pitanje vizualizacije ili jednostavno ideja — pošalji kontekst pa možemo krenuti odatle."
  },
  "contact.section": {
    "en": "01 / Get in touch",
    "tr": "01 / İletişime geç",
    "bs": "01 / Javi se"
  },
  "contact.sectionTitle": {
    "en": "Start with the <em>question.</em>",
    "tr": "<em>Soruyla</em> başla.",
    "bs": "Počni s <em>pitanjem.</em>"
  },
  "contact.emailTitle": {
    "en": "For project conversations, collaboration or opportunities.",
    "tr": "Proje konuşmaları, işbirliği veya fırsatlar için.",
    "bs": "Za razgovore o projektima, saradnju ili prilike."
  },
  "contact.emailP": {
    "en": "Tell me what you are working on, the place or problem involved, and what made you reach out. Useful context is enough.",
    "tr": "Ne üzerinde çalıştığını, ilgili alanı veya problemi ve neden iletişime geçtiğini anlat. Faydalı bir bağlam yeterli.",
    "bs": "Reci na čemu radiš, koja je lokacija ili problem i zašto mi se javljaš. Dovoljan je koristan kontekst."
  },
  "contact.linkedinTitle": {
    "en": "Professional conversations.",
    "tr": "Profesyonel görüşmeler.",
    "bs": "Profesionalni razgovori."
  },
  "contact.linkedinP": {
    "en": "Landscape architecture, internships, collaboration and design-related opportunities.",
    "tr": "Peyzaj mimarlığı, staj, işbirliği ve tasarım odaklı fırsatlar.",
    "bs": "Pejzažna arhitektura, prakse, saradnja i prilike vezane za dizajn."
  },
  "contact.githubTitle": {
    "en": "Digital work & experiments.",
    "tr": "Dijital çalışmalar ve denemeler.",
    "bs": "Digitalni rad i eksperimenti."
  },
  "contact.githubP": {
    "en": "Code, websites and experiments that sit alongside my spatial and visual work.",
    "tr": "Mekânsal ve görsel çalışmalarımın yanında geliştirdiğim kod, web ve deneysel işler.",
    "bs": "Kod, web stranice i eksperimenti koji prate moj prostorni i vizualni rad."
  },
  "contact.footerCta": {
    "en": "Send the context.<br><em>I’ll take a look.</em>",
    "tr": "Bağlamı gönder.<br><em>Birlikte bakalım.</em>",
    "bs": "Pošalji kontekst.<br><em>Pogledat ću.</em>"
  },
  "contact.footerText": {
    "en": "Email directly, connect on LinkedIn, or look through the portfolio first.",
    "tr": "Doğrudan e-posta gönder, LinkedIn’den bağlan veya önce portfolyoya göz at.",
    "bs": "Pošalji email, poveži se preko LinkedIna ili prvo pogledaj portfolio."
  },
  "portfolio.kicker": {
    "en": "SELECTED WORK · 07 PROJECTS",
    "tr": "SEÇİLİ ÇALIŞMALAR · 07 PROJE",
    "bs": "ODABRANI RADOVI · 07 PROJEKATA"
  },
  "portfolio.title": {
    "en": "Landscape at<br><em>different scales.</em>",
    "tr": "Farklı ölçeklerde<br><em>peyzaj.</em>",
    "bs": "Pejzaž na<br><em>različitim skalama.</em>"
  },
  "portfolio.copy": {
    "en": "Seven projects moving between public space, planning, residential landscape, technical design and competition work. Different briefs, but the same interest in how a site can become clearer, more useful and more memorable.",
    "tr": "Kamusal alan, planlama, konut peyzajı, teknik tasarım ve yarışma çalışmaları arasında ilerleyen yedi proje. Farklı briefler; fakat bir alanın nasıl daha net, daha kullanışlı ve daha akılda kalıcı hale gelebileceğine yönelik aynı merak.",
    "bs": "Sedam projekata između javnog prostora, planiranja, stambenog pejzaža, tehničkog dizajna i konkursnih radova. Različiti zadaci, ali isto interesovanje za to kako lokacija može postati jasnija, korisnija i upečatljivija."
  },
  "portfolio.section": {
    "en": "01 / Project index",
    "tr": "01 / Proje dizini",
    "bs": "01 / Indeks projekata"
  },
  "portfolio.sectionTitle": {
    "en": "The work, <em>one project at a time.</em>",
    "tr": "Çalışmalar, <em>tek tek projeler üzerinden.</em>",
    "bs": "Radovi, <em>projekat po projekat.</em>"
  },
  "portfolio.sectionText": {
    "en": "Open a project to see its drawings, visualizations and design thinking.",
    "tr": "Çizimleri, görselleştirmeleri ve tasarım düşüncesini görmek için bir projeyi aç.",
    "bs": "Otvori projekat da vidiš crteže, vizualizacije i dizajnersko razmišljanje."
  },
  "portfolio.footerCta": {
    "en": "Seven projects.<br><em>One evolving direction.</em>",
    "tr": "Yedi proje.<br><em>Gelişen tek bir yön.</em>",
    "bs": "Sedam projekata.<br><em>Jedan pravac koji se razvija.</em>"
  },
  "portfolio.footerText": {
    "en": "The web portfolio is built for exploration. The PDF keeps the original document sequence.",
    "tr": "Web portfolyo keşfetmek için tasarlandı. PDF ise orijinal belge sırasını koruyor.",
    "bs": "Web portfolio je napravljen za istraživanje. PDF zadržava originalni redoslijed dokumenta."
  },
  "common.viewWork": {
    "en": "View portfolio ↗",
    "tr": "Portfolyoyu incele ↗",
    "bs": "Pogledaj portfolio ↗"
  },
  "common.contact": {
    "en": "Contact me ↗",
    "tr": "İletişime geç ↗",
    "bs": "Javi mi se ↗"
  },
  "common.pdf": {
    "en": "Portfolio PDF ↗",
    "tr": "Portfolyo PDF ↗",
    "bs": "Portfolio PDF ↗"
  },
  "common.next": {
    "en": "Next project",
    "tr": "Sonraki proje",
    "bs": "Sljedeći projekat"
  },
  "common.story": {
    "en": "Project story",
    "tr": "Proje hikâyesi",
    "bs": "Priča projekta"
  },
  "common.visuals": {
    "en": "Drawings + visuals",
    "tr": "Çizimler + görseller",
    "bs": "Crteži + vizuali"
  },
  "common.type": {
    "en": "Type",
    "tr": "Tür",
    "bs": "Tip"
  },
  "common.context": {
    "en": "Context",
    "tr": "Bağlam",
    "bs": "Kontekst"
  },
  "common.focus": {
    "en": "Focus",
    "tr": "Odak",
    "bs": "Fokus"
  },
  "common.back": {
    "en": "Back to portfolio",
    "tr": "Portfolyoya dön",
    "bs": "Nazad na portfolio"
  },
  "common.open": {
    "en": "Open image",
    "tr": "Görseli aç",
    "bs": "Otvori sliku"
  },
  "common.openHelp": {
    "en": "Open any image to inspect it at a larger scale.",
    "tr": "Daha büyük ölçekte incelemek için herhangi bir görseli aç.",
    "bs": "Otvori bilo koju sliku da je pregledaš u većem formatu."
  },
  "common.footer": {
    "en": "Landscape Architecture · Spatial Design · Visualization",
    "tr": "Peyzaj Mimarlığı · Mekânsal Tasarım · Görselleştirme",
    "bs": "Pejzažna arhitektura · Prostorni dizajn · Vizualizacija"
  },
  "home.statProjects": {
    "en": "Projects",
    "tr": "Projeler",
    "bs": "Projekti"
  },
  "home.statLanguages": {
    "en": "Languages",
    "tr": "Diller",
    "bs": "Jezici"
  },
  "home.statLandscape": {
    "en": "Landscape Architecture",
    "tr": "Peyzaj Mimarlığı",
    "bs": "Pejzažna arhitektura"
  },
  "home.chipDesign": {
    "en": "Landscape Design",
    "tr": "Peyzaj Tasarımı",
    "bs": "Pejzažni dizajn"
  },
  "home.chipPlanning": {
    "en": "Planning",
    "tr": "Planlama",
    "bs": "Planiranje"
  },
  "home.chipVisualization": {
    "en": "Visualization",
    "tr": "Görselleştirme",
    "bs": "Vizualizacija"
  },
  "portfolio.projectsCta": {
    "en": "Projects ↓",
    "tr": "Projeler ↓",
    "bs": "Projekti ↓"
  },
  "common.selectedProject": {
    "en": "Selected Project",
    "tr": "Seçili Proje",
    "bs": "Odabrani projekat"
  },
  "contact.copyButton": {
    "en": "Copy email",
    "tr": "E-postayı kopyala",
    "bs": "Kopiraj email"
  },
  "title.home": {
    "en": "Mirza Dugopoljac | Landscape Architecture",
    "tr": "Mirza Dugopoljac | Peyzaj Mimarlığı",
    "bs": "Mirza Dugopoljac | Pejzažna arhitektura"
  },
  "title.about": {
    "en": "About | Mirza Dugopoljac",
    "tr": "Hakkımda | Mirza Dugopoljac",
    "bs": "O meni | Mirza Dugopoljac"
  },
  "title.contact": {
    "en": "Contact | Mirza Dugopoljac",
    "tr": "İletişim | Mirza Dugopoljac",
    "bs": "Kontakt | Mirza Dugopoljac"
  },
  "title.portfolio": {
    "en": "Portfolio | Mirza Dugopoljac",
    "tr": "Portfolyo | Mirza Dugopoljac",
    "bs": "Portfolio | Mirza Dugopoljac"
  },
  "new.selected": {
    "en": "Selected work",
    "tr": "Seçili projeler",
    "bs": "Odabrani radovi"
  },
  "new.workTitle": {
    "en": "Ideas grounded<br>in <em>place.</em>",
    "tr": "Yerle bağ kuran<br><em>fikirler.</em>",
    "bs": "Ideje ukorijenjene<br>u <em>prostoru.</em>"
  },
  "new.all": {
    "en": "Explore all 7 projects ↗",
    "tr": "7 projenin tamamını incele ↗",
    "bs": "Pogledaj svih 7 projekata ↗"
  },
  "new.cv": {
    "en": "Download CV ↓",
    "tr": "Özgeçmişi indir ↓",
    "bs": "Preuzmi CV ↓"
  },
  "new.pdf": {
    "en": "Portfolio PDF ↓",
    "tr": "Portfolyo PDF ↓",
    "bs": "Portfolio PDF ↓"
  },
  "new.intro": {
    "en": "A little about me",
    "tr": "Kısaca ben",
    "bs": "Ukratko o meni"
  },
  "new.introTitle": {
    "en": "From reading a landscape<br>to <em>shaping its future.</em>",
    "tr": "Peyzajı okumaktan<br><em>geleceğini şekillendirmeye.</em>",
    "bs": "Od razumijevanja pejzaža<br>do <em>oblikovanja budućnosti.</em>"
  },
  "new.talk": {
    "en": "Let’s start<br><em>a conversation.</em>",
    "tr": "Bir sohbetle<br><em>başlayalım.</em>",
    "bs": "Započnimo<br><em>razgovor.</em>"
  },
  "new.talkText": {
    "en": "Have a project, a collaboration or an opportunity in mind? I’d love to hear about it.",
    "tr": "Bir proje, iş birliği veya fırsat hakkında konuşmak ister misiniz? Sizden haber almak isterim.",
    "bs": "Imate projekat, saradnju ili priliku na umu? Volio bih čuti više."
  },
  "new.email": {
    "en": "Get in touch ↗",
    "tr": "İletişime geç ↗",
    "bs": "Javi se ↗"
  },
  "new.allFilter": {
    "en": "All projects",
    "tr": "Tüm projeler",
    "bs": "Svi projekti"
  },
  "new.landscape": {
    "en": "Landscape",
    "tr": "Peyzaj",
    "bs": "Pejzaž"
  },
  "new.planning": {
    "en": "Planning & GIS",
    "tr": "Planlama ve CBS",
    "bs": "Planiranje i GIS"
  },
  "new.detail": {
    "en": "Objects & details",
    "tr": "Nesneler ve detaylar",
    "bs": "Objekti i detalji"
  },
  "new.role": {
    "en": "Landscape architecture student",
    "tr": "Peyzaj mimarlığı öğrencisi",
    "bs": "Student pejzažne arhitekture"
  },
  "new.menu": {
    "en": "Menu",
    "tr": "Menü",
    "bs": "Meni"
  },
  "new.skip": {
    "en": "Skip to content",
    "tr": "İçeriğe geç",
    "bs": "Pređi na sadržaj"
  },
  "new.close": {
    "en": "Close image",
    "tr": "Görseli kapat",
    "bs": "Zatvori sliku"
  },
  "new.downloads": {
    "en": "Take a closer look",
    "tr": "Daha yakından incele",
    "bs": "Pogledaj detaljnije"
  },
  "new.education": {
    "en": "Education",
    "tr": "Eğitim",
    "bs": "Obrazovanje"
  },
  "new.design": {
    "en": "Landscape design",
    "tr": "Peyzaj tasarımı",
    "bs": "Pejzažni dizajn"
  },
  "new.gis": {
    "en": "GIS & planning",
    "tr": "CBS ve planlama",
    "bs": "GIS i planiranje"
  },
  "new.visualization": {
    "en": "Visualization",
    "tr": "Görselleştirme",
    "bs": "Vizualizacija"
  },
  "ed.identity": {
    "en": "Mirza Dugopoljac / Landscape architecture",
    "tr": "Mirza Dugopoljac / Peyzaj mimarlığı",
    "bs": "Mirza Dugopoljac / Pejzažna arhitektura"
  },
  "ed.work": {
    "en": "View the work ↗",
    "tr": "Projeleri incele ↗",
    "bs": "Pogledaj radove ↗"
  },
  "ed.about": {
    "en": "The approach",
    "tr": "Yaklaşım",
    "bs": "Pristup"
  },
  "ed.contact": {
    "en": "Start a conversation",
    "tr": "Bir sohbet başlat",
    "bs": "Započni razgovor"
  },
  "ed.layerIntro": {
    "en": "One landscape. Three ways of seeing.",
    "tr": "Tek peyzaj. Üç farklı bakış.",
    "bs": "Jedan pejzaž. Tri načina gledanja."
  },
  "ed.plan": {
    "en": "Plan",
    "tr": "Plan",
    "bs": "Plan"
  },
  "ed.experience": {
    "en": "Experience",
    "tr": "Deneyim",
    "bs": "Doživljaj"
  },
  "ed.analysis": {
    "en": "Analysis",
    "tr": "Analiz",
    "bs": "Analiza"
  },
  "ed.planLabel": {
    "en": "01 / Spatial structure",
    "tr": "01 / Mekânsal yapı",
    "bs": "01 / Prostorna struktura"
  },
  "ed.planTitle": {
    "en": "An idea, made legible.",
    "tr": "Okunabilir bir fikir.",
    "bs": "Ideja koja se može čitati."
  },
  "ed.planCopy": {
    "en": "Paths, planted edges and gathering spaces. The plan makes the relationships visible.",
    "tr": "Yollar, bitkisel sınırlar ve buluşma alanları. Plan, ilişkileri görünür kılar.",
    "bs": "Staze, zasađeni rubovi i prostori okupljanja. Plan čini odnose vidljivim."
  },
  "ed.experienceLabel": {
    "en": "02 / Atmosphere & use",
    "tr": "02 / Atmosfer ve kullanım",
    "bs": "02 / Atmosfera i upotreba"
  },
  "ed.experienceTitle": {
    "en": "A place to spend time.",
    "tr": "Zaman geçirilecek bir yer.",
    "bs": "Mjesto za boravak."
  },
  "ed.experienceCopy": {
    "en": "Light, water, material and movement. A drawing becomes an everyday experience.",
    "tr": "Işık, su, malzeme ve hareket. Bir çizim gündelik deneyime dönüşür.",
    "bs": "Svjetlo, voda, materijal i kretanje. Crtež postaje svakodnevni doživljaj."
  },
  "ed.analysisLabel": {
    "en": "03 / Reading the landscape",
    "tr": "03 / Peyzajı okumak",
    "bs": "03 / Čitanje pejzaža"
  },
  "ed.analysisTitle": {
    "en": "Every decision starts somewhere.",
    "tr": "Her kararın bir başlangıcı var.",
    "bs": "Svaka odluka negdje počinje."
  },
  "ed.analysisCopy": {
    "en": "Terrain, water and ecological patterns. GIS layers reveal what a site is already telling us.",
    "tr": "Arazi, su ve ekolojik örüntüler. CBS katmanları alanın bize anlattıklarını ortaya çıkarır.",
    "bs": "Teren, voda i ekološki obrasci. GIS slojevi otkrivaju ono što nam lokacija već govori."
  },
  "ed.open": {
    "en": "Explore the case study ↗",
    "tr": "Projeyi keşfet ↗",
    "bs": "Istraži projekat ↗"
  },
  "ed.thesisLabel": {
    "en": "01 / The starting point",
    "tr": "01 / Başlangıç noktası",
    "bs": "01 / Polazna tačka"
  },
  "ed.thesis": {
    "en": "A landscape isn’t an empty canvas. It is a living system of water, movement and memory. <em>Design starts with paying attention.</em>",
    "tr": "Peyzaj boş bir tuval değildir. Su, hareket ve hafızadan oluşan canlı bir sistemdir. <em>Tasarım, dikkatle bakmakla başlar.</em>",
    "bs": "Pejzaž nije prazno platno. To je živi sistem vode, kretanja i sjećanja. <em>Dizajn počinje pažljivim posmatranjem.</em>"
  },
  "ed.proof1": {
    "en": "Site before form",
    "tr": "Biçimden önce alan",
    "bs": "Lokacija prije forme"
  },
  "ed.proof1copy": {
    "en": "Terrain, ecology and everyday use come before the first design decision.",
    "tr": "Arazi, ekoloji ve gündelik kullanım ilk tasarım kararından önce gelir.",
    "bs": "Teren, ekologija i svakodnevna upotreba prethode prvoj projektnoj odluci."
  },
  "ed.proof2": {
    "en": "Across scales",
    "tr": "Ölçekler arasında",
    "bs": "Kroz različite skale"
  },
  "ed.proof2copy": {
    "en": "From a city’s green infrastructure to the detail of a place to sit.",
    "tr": "Bir kentin yeşil altyapısından bir oturma alanının detayına kadar.",
    "bs": "Od zelene infrastrukture grada do detalja mjesta za sjedenje."
  },
  "ed.proof3": {
    "en": "Drawn, tested, explained",
    "tr": "Çizildi, denendi, açıklandı",
    "bs": "Nacrtano, ispitano, objašnjeno"
  },
  "ed.proof3copy": {
    "en": "Plans, sections and visual studies make the thinking open to inspection.",
    "tr": "Planlar, kesitler ve görsel çalışmalar tasarım düşüncesini incelemeye açar.",
    "bs": "Planovi, presjeci i vizualne studije omogućavaju uvid u razmišljanje."
  },
  "ed.doors": {
    "en": "02 / Three ways in",
    "tr": "02 / Üç giriş noktası",
    "bs": "02 / Tri polazišta"
  },
  "ed.door1": {
    "en": "The landscape",
    "tr": "Peyzaj",
    "bs": "Pejzaž"
  },
  "ed.door1copy": {
    "en": "Public spaces, residential gardens and shared ground. Explore the projects through plans and lived atmosphere.",
    "tr": "Kamusal alanlar, konut bahçeleri ve ortak zeminler. Projeleri planlar ve mekânsal atmosfer üzerinden keşfedin.",
    "bs": "Javni prostori, stambeni vrtovi i zajedničko tlo. Istražite projekte kroz planove i atmosferu prostora."
  },
  "ed.door2": {
    "en": "The layers",
    "tr": "Katmanlar",
    "bs": "Slojevi"
  },
  "ed.door2copy": {
    "en": "Site analysis and GIS — reading natural and cultural systems before deciding what should change.",
    "tr": "Alan analizi ve CBS — neyin değişmesi gerektiğine karar vermeden önce doğal ve kültürel sistemleri okumak.",
    "bs": "Analiza lokacije i GIS — čitanje prirodnih i kulturnih sistema prije odluke o promjeni."
  },
  "ed.door3": {
    "en": "The details",
    "tr": "Detaylar",
    "bs": "Detalji"
  },
  "ed.door3copy": {
    "en": "Small structures, urban furniture and technical studies. Looking closer at how an idea meets everyday use.",
    "tr": "Küçük yapılar, kent mobilyaları ve teknik çalışmalar. Bir fikrin gündelik kullanımla buluşmasına yakından bakmak.",
    "bs": "Male strukture, urbani namještaj i tehničke studije. Detaljniji pogled na susret ideje i svakodnevne upotrebe."
  },
  "ed.capabilities": {
    "en": "03 / Capability index",
    "tr": "03 / Çalışma alanları",
    "bs": "03 / Područja rada"
  },
  "ed.cap1": {
    "en": "Landscape & public space",
    "tr": "Peyzaj ve kamusal alan",
    "bs": "Pejzaž i javni prostor"
  },
  "ed.cap1copy": {
    "en": "Spatial concepts, circulation, planting and shared outdoor life.",
    "tr": "Mekânsal kavramlar, dolaşım, bitkilendirme ve ortak açık alan yaşamı.",
    "bs": "Prostorni koncepti, kretanje, sadnja i zajednički život na otvorenom."
  },
  "ed.cap2": {
    "en": "Planning & GIS",
    "tr": "Planlama ve CBS",
    "bs": "Planiranje i GIS"
  },
  "ed.cap2copy": {
    "en": "Natural and cultural analysis, mapping and landscape strategy.",
    "tr": "Doğal ve kültürel analiz, haritalama ve peyzaj stratejisi.",
    "bs": "Prirodna i kulturna analiza, kartiranje i pejzažna strategija."
  },
  "ed.cap3": {
    "en": "Technical design",
    "tr": "Teknik tasarım",
    "bs": "Tehnički dizajn"
  },
  "ed.cap3copy": {
    "en": "Plans, sections and details that connect the idea to its construction.",
    "tr": "Fikri yapımla ilişkilendiren planlar, kesitler ve detaylar.",
    "bs": "Planovi, presjeci i detalji koji povezuju ideju s izvedbom."
  },
  "ed.cap4": {
    "en": "Spatial visualization",
    "tr": "Mekânsal görselleştirme",
    "bs": "Prostorna vizualizacija"
  },
  "ed.cap4copy": {
    "en": "Models and images to explore material, light and atmosphere.",
    "tr": "Malzeme, ışık ve atmosferi araştıran modeller ve görseller.",
    "bs": "Modeli i slike za istraživanje materijala, svjetla i atmosfere."
  },
  "ed.registry": {
    "en": "04 / Selected work registry",
    "tr": "04 / Seçili proje dizini",
    "bs": "04 / Registar odabranih radova"
  },
  "ed.registryTitle": {
    "en": "The thinking.<br><em>On the ground.</em>",
    "tr": "Düşüncenin<br><em>zemindeki karşılığı.</em>",
    "bs": "Razmišljanje.<br><em>Na terenu.</em>"
  },
  "ed.case": {
    "en": "Open case file ↗",
    "tr": "Proje dosyasını aç ↗",
    "bs": "Otvori projekat ↗"
  },
  "ed.practice": {
    "en": "05 / The practice",
    "tr": "05 / Yaklaşımım",
    "bs": "05 / Praksa"
  },
  "ed.practiceTitle": {
    "en": "Curiosity is<br><em>a working method.</em>",
    "tr": "Merak bir<br><em>çalışma yöntemidir.</em>",
    "bs": "Radoznalost je<br><em>metoda rada.</em>"
  },
  "ed.notes": {
    "en": "06 / From the project files",
    "tr": "06 / Proje dosyalarından",
    "bs": "06 / Iz projektnih bilješki"
  },
  "ed.notesTitle": {
    "en": "Look a little closer.",
    "tr": "Biraz daha yakından bak.",
    "bs": "Pogledaj malo bliže."
  },
  "ed.contactLabel": {
    "en": "07 / Direct contact",
    "tr": "07 / Doğrudan iletişim",
    "bs": "07 / Direktni kontakt"
  },
  "ed.pause": {
    "en": "Pause field",
    "tr": "Alanı duraklat",
    "bs": "Pauziraj polje"
  },
  "ed.resume": {
    "en": "Resume field",
    "tr": "Alanı sürdür",
    "bs": "Pokreni polje"
  },
  "ed.field": {
    "en": "Contour study / illustrative terrain",
    "tr": "Eşyükselti çalışması / temsili arazi",
    "bs": "Studija izohipsi / ilustrativni teren"
  },
  "ed.index": {
    "en": "Portfolio index",
    "tr": "Portfolyo dizini",
    "bs": "Indeks portfolija"
  },
  "ed.projects": {
    "en": "Project records",
    "tr": "Proje kaydı",
    "bs": "Projektnih zapisa"
  },
  "ed.drawings": {
    "en": "Project visuals",
    "tr": "Proje görseli",
    "bs": "Projektnih vizuala"
  },
  "ed.languages": {
    "en": "Reading languages",
    "tr": "Okuma dili",
    "bs": "Jezika"
  },
  "ed.location": {
    "en": "Bursa, Türkiye / Landscape architecture",
    "tr": "Bursa, Türkiye / Peyzaj mimarlığı",
    "bs": "Bursa, Türkiye / Pejzažna arhitektura"
  },
  "ed.doorTag1": {
    "en": "Public space · Landscape",
    "tr": "Kamusal alan · Peyzaj",
    "bs": "Javni prostor · Pejzaž"
  },
  "ed.doorTag2": {
    "en": "Site analysis · GIS",
    "tr": "Alan analizi · CBS",
    "bs": "Analiza lokacije · GIS"
  },
  "ed.doorTag3": {
    "en": "Objects · Technical design",
    "tr": "Nesneler · Teknik tasarım",
    "bs": "Objekti · Tehnički dizajn"
  },
  "ed.capTag1": {
    "en": "Plan / Plant / Connect",
    "tr": "Planla / Bitkilendir / Bağla",
    "bs": "Planiraj / Zasadi / Poveži"
  },
  "ed.capTag2": {
    "en": "Read / Map / Synthesize",
    "tr": "Oku / Haritala / Sentezle",
    "bs": "Čitaj / Kartiraj / Poveži"
  },
  "ed.capTag3": {
    "en": "Draw / Test / Refine",
    "tr": "Çiz / Dene / Geliştir",
    "bs": "Crtaj / Ispitaj / Doradi"
  },
  "ed.capTag4": {
    "en": "Model / Light / Compose",
    "tr": "Modelle / Aydınlat / Düzenle",
    "bs": "Modeliraj / Osvijetli / Oblikuj"
  }
};
